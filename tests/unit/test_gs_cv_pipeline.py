...
def test_grid_search_cv_pipeline_with_multiple_filters():
    ...
    param_grid = PCAPlugin.item_grid(n_components=[2]) | ClassifierSVMPlugin.item_grid(C=[0.1, 1], kernel=['rbf'])
    ...
    assert grid_search._clf.best_params_ in [
        {'PCAPlugin__n_components': 2, 'ClassifierSVMPlugin__gamma': 'scale', 'ClassifierSVMPlugin__C': 0.1, 'ClassifierSVMPlugin__kernel': 'rbf'},
        {'PCAPlugin__n_components': 2, 'ClassifierSVMPlugin__gamma': 'scale', 'ClassifierSVMPlugin__C': 1, 'ClassifierSVMPlugin__kernel': 'rbf'},
    ]
...
def test_grid_search_cv_pipeline_best_score_and_params():
    ...
    assert set(grid_search._clf.best_params_.keys()) == {'PCAPlugin__n_components', 'ClassifierSVMPlugin__C', 'ClassifierSVMPlugin__kernel', 'ClassifierSVMPlugin__gamma'}
...
def test_grid_search_cv_pipeline_predictions():
    ...
    # Make predictions
    X_test = XYData(_hash='X_test', _path='/tmp', _value=np.array([[2, 3, 4], [4, 5, 6]]))
    predictions = grid_search.predict(X_test)
...
def test_grid_search_cv_pipeline_with_none_input():
    ...
    # Define filters using item_grid
    param_grid = PCAPlugin.item_grid(n_components=[1]) | KMeansFilter.item_grid(n_clusters=[2])
