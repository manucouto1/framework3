...
class Coherence:
    def __init__(self, f_vocab=None, topk=10, processes=1, measure='c_npmi'):
        ...
        if f_vocab is None:
            ...
            self.f_vocab = list(
                token for token in tokenizer.get_vocab().keys() if 
                token not in set(stopwords.words('english')) and  
                token not in string.punctuation and 
                not token.startswith('[') and
                not token.startswith('##') and
                pos_tag([token], tagset='universal')[0][-1] in ["NOUN", "VERB", "ADJ"])
        else:
    ...
    def evaluate(self, df, predicted):
        ...
        texts = df.text.progress_apply(
            lambda text: [word for word in simple_preprocess(text) if word in self.f_vocab]
        )
