...
class GensimEmbedder(BaseFilter):
    ...
    def predict(self, x: XYData) -> XYData:
        self._embeddings = self._embeddings()  # Load the embeddings if they haven't been loaded yet
        all_m = []
...
class TransformersEmbedder(BaseFilter):
    def __init__(self, model_path: str):
        ...
        self.model_path = model_path  # Path to the pre-trained model (e.g., 'bert-base-uncased')
    ...
    def predict(self, x: XYData) -> XYData:
        ...
        for topic in tqdm(x.value):
            ...
            for word in topic:
                ...
                mean_embeddings = torch.mean(out.last_hidden_state[0][1:-1].detach().cpu(), axis=0) # type: ignore
...
class PerTopicMeanSimilarity(BaseFilter):
    ...
    def predict(self, x: XYData) -> XYData:
        ...
        for i in range(topic_tensors.size(0)):
            top_sim = 1 - pairwise_cosine_similarity(topic_tensors[i], topic_tensors[i])
        ...
        return XYData.mock(1 - torch.mean(torch.stack(topic_results), dim=1))
...
for name, config in datasets.items():
    ...
    X = XYData(_hash=name,_path='datasets',_value=df.loc[:,df.columns[1:]].values) # type: ignore
    ...
    df = pd.DataFrame(preduction2.value, columns=transformers_models+gensim_models)
